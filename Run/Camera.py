...
class SaveWorker(Process):
    ...
    def run(self):
        idx = 0
        while True:
            #continuosly grab files from the save queue for saving
            ...
            if idx % 10 == 0: #show preview every 10th frame only
...
class CameraWorker(QtCore.QThread):
    ...
    def __init__(self, cam, folder_base:str, base_name:str, motor_step:Numeric, azimuth_step:Numeric, metadata:dict, display_queue,motor, encoder, stepper): 
        super().__init__()
        self.cam = cam
    ...
    def run(self):
        ...
        save_queue = JoinableQueue(maxsize=self.total_frames) #make sure that only the amount of frames for one scan can be saved at a time
        ...
        try:
            self.motor.setSpeed(120) #can change this so motor speed is slower or faster
            for i in self.total_iterations: #azimuth angles
                folder = self.folder_base / f"{self.base_name}_{i:06.2f}"
                folder.mkdir(parents=True, exist_ok=True)
                frame_idx = 0
                for j in self.target_angles: #motor angles
                    if self._stop:
                        break
                    while True:
                        degree = self.encoder.readDeg() #read Degree angle from the encoder
                        if abs(degree - j) < 0.0675: #tolerance for the angle, can change
                            break
...
class CalibrateDialog(QtWidgets.QDialog):
    ...
    def calibrate(self):
        self.cam.stop()
        #calibration allows the camera to take a picture and use it's own auto selection for controls
...
class CaptureApp(QtWidgets.QWidget):
    ...
    def start_capture(self):
        self.status.clear()
        self.start_btn.setEnabled(False) #dont let someone reclick start capture button once its has started running
